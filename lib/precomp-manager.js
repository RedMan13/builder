...
class PrecompManager {
    ...
    async recursiveRead(dir = this.buildDir) {
        ...
        for (const file of dirs) {
            ...
            if (file.includes('.prot1') || file.includes('.prot2') || file.includes('.prot3')) continue;
            if (file.endsWith('.php') || file.endsWith('.html') || file.endsWith('.server.js')) {
                const content = await fs.readFile(target, 'utf8');
                const title = content.match(/<title>(.*?)<\/title>/i)?.[1] ?? file;
                // a server file without any title is most likely not relevant
                if (file.endsWith('.server.js') && title === file) continue;
