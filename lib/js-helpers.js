...
function toCJS(data) {
    const toks = new Tokenizer(data, {
        ...
        export(str) {
            ...
            const fromIdx = str.indexOf('from') < withStr && str.indexOf('from') > 0 
                ? str.indexOf('from') +4 
                : withIdx -4;
            const fromStr = str.slice(fromIdx, withStr -4).trim();
    ...
    for (const tok of tokens) {
        ...
        case 'import': {
            ...
            replace += `\nconst ${cont} = require(${JSON.stringify(tok.path)}, ${JSON.stringify(tok.props)}); `;
...
module.exports = ${variable};\n`;
    ...
    const imported = tokens
        .filter(tok => tok.name === 'import' || tok.imports)
        .map(imp => [imp.path || imp.imports, imp.props]);
