...
function toCJS(data) {
    const toks = new Tokenizer(data, {
        ...
        import(str) {
            ...
            const len = str.length - str.trimStart().length;
        ...
        export(str) {
            ...
            const len = str.length - str.trimStart().length;
            str = str.trimStart();
            const afterExport = str.slice(6).trimStart();
...
module.exports = ${variable};\n`;
    ...
    return data.replace(/(let|var|const)\s+(__dirname|__filename)\s*=\s*.*?(;|\n)/g, '');
}
...
function getImported(data) {
    ...
    if (data.length >= 50000) {
        ...
        ].map(m => ({ start: m.index, ...m.groups }));
    } else {
        const parser = new Tokenizer(data, {
            _(str) {
                const jmp = jumpArbit(str);
                return jmp ? { length: jmp } : null;
            },
    ...
    return toks
        ...
        .map(tok => [
            tok.start + tok.statement.length, 
            tok.start + tok.statement.length + tok.module.length, 
            parseStringAt(tok.module)
        ])
        .filter(Boolean);
}
...
async function getDeepFiles(file, manager, handled = {}) {
    ...
    handled[file] = true;
    if (!manager.built[file]) delete cache[file];
    if (cache[file]) {
        if (cache[file].every(imp => manager.built[imp[0]]))
            return [...cache[file]]
    }
    console.log(`\t\t\ttraversing imports of ${path.relative(manager.entry, file)}`);
    
    let [real, data] = manager.path === file 
        ? [file.replace(manager.entry.slice(0, -1), manager.buildDir), manager.file]
        : await manager.getFile(file);
    const ret = [[real, data]];
    ...
    for (const [start, end, imported] of imports) {
        ...
        data = data.slice(0, start + off) + JSON.stringify(imp) + data.slice(end + off);
        off += (imp.length + 2) - (end - start);
